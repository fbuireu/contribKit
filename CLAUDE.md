...
`docs/docs-consistency.test.ts` makes the mechanical half executable: it reads every document as data and asserts the checkable claims against the repo — no dead relative link, no citation of a source file that does not exist, no `file.ts:123` line citation (they rot the moment anything above them moves; name the symbol instead), `shared/*.json` equal to the mirrored copies in `app/assets/` once line endings and trailing whitespace are normalised, because a Windows checkout rewrites them, every palette and shape shipped named in the README's own feature line — not merely present somewhere in the file, which made `GitHub` and `square` unfailable, every pinned version matching the manifest that pins it, every documented `pnpm` script declared in a `package.json`, every source layer carrying a nested `CLAUDE.md` with no stray `CONTEXT.md` outside the root, nothing under `web/src/pages` becoming a public URL by accident, every bare filename a nested guide names still existing somewhere in the source — including `app/test`, `web/e2e` and `web/workers`, because a guide that points at the test pinning a rule is citing the most useful file it could, every glossary term used somewhere outside the glossary, no identifier in `web/src` or `app/lib` named after a word a glossary `_Avoid_` list rejects — both the code-shaped terms (`ShapeKind`, `DOW`, `IAP`, `SKU`) anywhere, and a curated set of plain words (`purchase`, `paywall`, `heatmap`, `donation`, …) in `.ts` and `.dart` with string literals stripped, so it reads identifiers rather than prose; a second assertion proves every policed word is one `CONTEXT.md` actually rejects, so the list cannot invent a rule. It is still not all 106 terms and cannot be: most are ordinary English (`value`, `range`, `save`) that any codebase uses honestly, and some name a platform API rather than our vocabulary — `showPopover` is the HTML Popover API, not a Cell Tooltip called the wrong thing. For the same reason the `SDK_SEAMS` list exempts the two files that speak to the store SDK, whose job is to talk the vendor's language on one side and the glossary's on the other. A third assertion caps that list and checks each file still exists, so the exemption cannot quietly grow. The stripper removes Dart raw strings before escape sequences and works one line at a time, so a regex literal cannot break quote pairing for the rest of a file, no `//` **or `/* */`** comment in any hand-written source — the block form was invisible for a year — `app/lib`, `app/test`, `web/src`, `web/e2e`, `web/workers`, `docs/` and `scripts/`, plus the three `web/*.config.ts`, everything but the generated files — the exception list is exactly `// @vitest-environment` and `/// <reference>`, the two the runner reads, so a `///` Dart doc comment or a `// @TODO` is caught like any other; the web layers importing only inwards, over `.astro` as well as `.ts` and over every import form, not just `from "…"` — that hole is how a marketing component reached around the domain and counted the raw token JSON instead of the filtered `CELL_SHAPES`; `shadcn_ui` confined to `app/lib/ui/widgets/`, the theme and the composition root — and the ADR set held to its template — sequential numbering, `NNNN-kebab-title.md`, the `# N. Title` / Date / Status / *Context* / *Decision* / *Consequences* shape, a row in the [`ARCHITECTURE.md`](./ARCHITECTURE.md) index, and a link from some document **other** than that index. It runs with `pnpm test` and on its own with `pnpm test:docs`. A failure means the docs and the code disagree — fix whichever is wrong, and never delete an assertion to make it pass. It cannot check prose or rationale; that part is still on you. Keep its assertions aggregated (one failing list per rule) rather than one case per document.
...
- **A guard that never runs is not a guard.** Both CI workflows are path-filtered, so the docs contract needs an entry point in each: `ci-web.yml` runs it because `docs/**`, `shared/**`, `scripts/**`, `*.md` and the root `package.json` / `pnpm-workspace.yaml` / `lefthook.yml` are in its trigger list, and `ci-app.yml` carries a `Docs Contract` job because nothing under `app/**` triggers the web workflow — and half the assertions are about the Flutter side. Removing either disables it silently. **This has now happened three times**: the app side, the preview-Worker cleanup, and then `scripts/**` plus the root manifest — the contract policed comments in `scripts/` and asserted the pnpm and Node pins that live in that manifest, and a change to either ran nothing at all. Whenever you add an assertion, ask which filter carries the files it reads. **The web filter is written three times and is now asserted equal** — `ci-web.yml`'s `paths`, `ci-web-noop.yml`'s `paths-ignore` and `cleanup-web-development.yml`'s `paths` — because the cleanup copy had drifted four entries behind, so a PR touching only `scripts/` or the root manifest created a preview Worker nothing deleted. Three documents claimed those filters mirror each other while one did not; the contract reads them now. And a guard that fails for a non-reason is not a guard either: this file's own assertions used to time out at five seconds under a parallel run while passing in four alone, so it sets its own `testTimeout`.

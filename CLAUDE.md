...
- pnpm **11.21.0**: always pnpm, never npm/yarn. The root `packageManager` is the **only** pin: it is what
  `pnpm/action-setup` resolves everywhere it runs, because `ci-web.yml` and `release-app.yml` pass

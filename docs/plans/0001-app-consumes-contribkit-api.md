...
2. **Shape mismatch.** The API returns a flat `cells` array with a nullable `count` and no year. The app's `ContributionCalendar` is weeks with a year and a non-nullable count. Either the API grows a representation the app can consume, or the app converts at its boundary.
3. **Level parity.** Now that both read GitHub's `data-level`, the API already carries the authoritative level per cell. Nothing more is needed here, but it must stay that way.

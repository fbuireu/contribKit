...
- The header set is no longer uniform. Anyone reading `SECURITY_HEADERS` alone now has an incomplete picture of what a given response carries. The override is a few lines below it, and the [API reference](https://github.com/fbuireu/contribKit/wiki/API-Reference) documents both values.

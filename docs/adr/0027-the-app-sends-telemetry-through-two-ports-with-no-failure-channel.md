...
- **`dart-defines.json` and `dart-defines.prod.json` now differ by two keys, not one.** [22](0022-the-app-has-no-build-flavors-and-the-stage-is-a-dart-defines-file.md) recorded that the difference was one string and said to reopen the decision when a second value had to vary per stage. `TELEMETRY_ENVIRONMENT` is that second value. It does not reopen it: the rule that mattered was "no Gradle flavor while the difference is expressible in Dart", and this is still expressible in Dart. That ADR's Context is now describing a past state and says so.

...
Accepted. Amended 2026-09-24: a Usage Event carries typed properties drawn from closed sets beside its name, on both clients; the *Decision* section carries the amendment and what it keeps. Sits beside [4](0004-typed-failures-instead-of-thrown-exceptions.md), whose rule it deliberately departs from, and [25](0025-how-much-ddd-and-where-it-stops.md), whose three questions decided what got modelled. Extended by [29](0029-diagnostic-reports-carry-a-masked-session-replay.md), which adds a masked replay to a foreground report without widening what one may carry.
...
**A Usage Event carries its name and typed properties, and no free text.** `UsageEvent` was a sealed enum in `domain/value_objects/` and `record` took one, not a string with a properties map, so there was no parameter through which a Username could be passed and no reviewer had to check that none was. Applying the three questions of [25](0025-how-much-ddd-and-where-it-stops.md): the illegal state is reachable (any string would have been accepted), something reads it (the adapter, and PostHog's dashboard), and it crosses a boundary (off the device). It earns a type.

*Amended 2026-09-24.* A name alone answered "was the Customizer opened" and could not answer "which Palette do people choose", "which Export Format leaves the app" or "which Year do people look at", which is what the events exist to answer. `UsageEvent` is a final class now, with one constructor per event whose parameters are domain enums, value objects and numbers: a `Palette` contributes its key, an `ExportFormat` its name, a `Year` its number, an outcome its enum case. The guarantee moved from "no properties" to "no `String` parameter": the type still admits no free text, so a Username still has no way in, and that is the property a reviewer checks. The web's `recordUsageEvent` is written to the same rule, with a typed properties map per event name. What the amendment does not change: `record` still takes a `UsageEvent` and nothing else, the adapter still swallows every error, and the published privacy policy names the properties an event may carry.

**A Diagnostic Report carries the error's type and its stack, never its message.** Since [29](0029-diagnostic-reports-carry-a-masked-session-replay.md) a foreground report also carries a replay in which every text, image and Cell is a rectangle, which is the same rule applied to pixels. The Sentry adapter's `beforeSend` rewrites the `value` of every `SentryException` to a constant and blanks `message` when there is one, so the rule holds for reports this code raises *and* for the unhandled errors `SentryFlutter.init` captures on its own, which is the path nobody writes and everybody forgets. It walks chained causes too, because a wrapped `Failure` keeps its own message one level down. Breadcrumbs are off at source (`maxBreadcrumbs = 0`) as well as cleared in `beforeSend`, so nothing rides on the hook alone. The first version of that hook was written against the 8.x SDK's `copyWith`, which is `x ?? this.x` and silently keeps a field you try to clear with null; the 9.x classes are mutable and the scrubber assigns directly, which is why the SDK moved to 9.x in the same change that made the Android build compile again (its 8.x Kotlin module declares language version 1.6, which Kotlin 2.4 refuses).
...
- **The release build is obfuscated, which makes the symbol upload load-bearing rather than a nicety.** A
  Diagnostic Report keeps the exception's type and drops its message, so the type is the *whole* of what a report
  says. `--obfuscate` renames classes, so without the symbols `sentry_dart_plugin` uploads, that type arrives as
  a meaningless identifier and the report says nothing at all. `release-app.yml` therefore **fails** when
  `SENTRY_AUTH_TOKEN` is absent rather than warning and continuing: shipping an obfuscated build with no symbols
  produces crash reports that look present and carry nothing. Enum names are **not** obfuscated by Dart, which is
  what lets `UsageEvent.name` stay the wire value and the Hive settings keep round-tripping through `.name`.
...
- Where this bites: [`app/lib/infrastructure/AGENTS.md`](../../app/lib/infrastructure/AGENTS.md), [`app/lib/domain/AGENTS.md`](../../app/lib/domain/AGENTS.md), [`app/lib/ui/di/AGENTS.md`](../../app/lib/ui/di/AGENTS.md), and the consent half in [28](0028-telemetry-consent-is-asked-twice-and-answered-asymmetrically.md).

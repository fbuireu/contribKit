...
The web does not have the concept at all. `SvgRenderOptions` in [`web/src/domain/services/types.ts`](../../web/src/domain/services/types.ts) carries `cellSize?: number` and `cellGap?: number`, defaulting to `SVG_DEFAULT_CELL_SIZE` and `SVG_DEFAULT_CELL_GAP`. Those numbers are not a user choice: they come from three fixed presets in [`web/src/ui/components/grid/grid-presets.ts`](../../web/src/ui/components/grid/grid-presets.ts) (hero, customize and export), each tuned to the space its surface has. The SVG endpoint's query schema accepts `palette`, `shape` and `background`, and nothing else.

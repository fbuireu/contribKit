...
Both signals leave through Cloudflare. [`web/wrangler.toml`](../../web/wrangler.toml) names `destinations` on `observability.logs` and `observability.traces` in both stages, sets `redact_query_string = true`, and puts `head_sampling_rate` at `1`: the ratio started at 0.2, which
dropped four invocations in five and with them the trace most failures belonged to.

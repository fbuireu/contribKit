...
**Amended 2026-09-04: it is held to the numbers by a test, if not by the compiler.** The consequence below called that an unclosable gap because the widget renders to a bitmap this project cannot assert on. That is true of the *output* and not of the *source*: the four numbers are literals in a file, and [`docs/docs-consistency.test.ts`](../docs-consistency.test.ts) reads all three languages and asserts they agree, the way it already diffs the Embed contract across two. A drift is a failing test now rather than a review someone has to remember to do.

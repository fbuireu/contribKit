...
- **The pairing with Kotlin is prose and a code review, and that is a real gap.** [`app/test/ui/features/widget/dart_kotlin_seam_test.dart`](../../app/test/ui/features/widget/dart_kotlin_seam_test.dart) covers the strings that cross that boundary and cannot cover the numbers, because the widget renders to a bitmap this project has no way to assert on. If the Cell ever looks wrong on the home screen and right everywhere else, this is where to look first.

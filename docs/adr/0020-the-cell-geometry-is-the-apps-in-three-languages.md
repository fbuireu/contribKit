...
**The web's miniatures are out of scope, and stay out.** `mini-grid.ts` (`CELL_SIZE = 4`), `ContributionCode.astro` (`CELL_SIZE = 18`) and `shapePreviewSVG` draw decorative shapes at fixed sizes rather than Cells at a chosen one, `web/src/ui/components/CLAUDE.md` lists those three. `code-preview.ts` is a fourth and a narrower case: it asks the geometry module for its radii and still computes its own view box, without the trailing-gap subtraction `calendarLayout` applies. This decision binds anything that renders a Contribution Calendar; a 20 by 20 swatch is not one.

...
2. **Does anything read it?** Modelling a concept nothing consumes invents a type whose only reader is its own test. The root [`AGENTS.md`](../../AGENTS.md) already names the shared-token-nothing-reads trap; a value object nothing reads is the same trap wearing a pattern's name.

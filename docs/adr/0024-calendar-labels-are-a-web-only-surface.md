...
Adding `MONTH_LABELS` and `WEEKDAY_LABELS` to `app/lib/domain/` without a renderer that reads them would create a token nothing consumes, which the root [`AGENTS.md`](../../AGENTS.md) names as a trap this repository has already fallen into once.

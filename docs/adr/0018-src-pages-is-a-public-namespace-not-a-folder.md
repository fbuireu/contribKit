...
The conventions: tests are colocated next to the code they cover, and every source layer carries a nested `AGENTS.md`. The second is not a preference but an assertion in [`docs/docs-consistency.test.ts`](../docs-consistency.test.ts), so the file *must* exist at [`web/src/pages/AGENTS.md`](../../web/src/pages/AGENTS.md).
...
Prefixing the guide itself was rejected: `_AGENTS.md` is not auto-loaded by an agent arriving in that folder, which is the entire reason the nested guides carry that name.
...
The pages guide stays at `web/src/pages/AGENTS.md`, and `AGENT_GUIDE_ROUTE` in [`web/src/middleware.ts`](../../web/src/middleware.ts) answers `404` for `/AGENTS` before the route runs. [`astro.config.ts`](../../web/astro.config.ts) also drops it from the sitemap. The page is still built (it is markdown, and small), but it is not reachable and not advertised.

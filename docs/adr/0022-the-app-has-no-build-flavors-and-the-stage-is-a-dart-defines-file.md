...
That convention is sized for a real difference between stages. When this was written the difference was **one string**: `dart-defines.json` and `dart-defines.prod.json` each held only `REVENUECAT_KEY`, read with `String.fromEnvironment`. [27](0027-the-app-sends-telemetry-through-two-ports-with-no-failure-channel.md) added `TELEMETRY_ENVIRONMENT` beside it, which does not reopen this decision because the rule that mattered was that no difference needs Gradle. Nothing else about the two builds differs: same `applicationId`, same signing config, same assets, same entitlements, same endpoints, because the app talks to GitHub's public page and to no backend of ours ([5](0005-scrape-githubs-public-contributions-html.md), [11](0011-keep-the-apps-own-scraper-for-now.md)).
...
- Where it bites: the **Gotchas** in [`CLAUDE.md`](../../CLAUDE.md), the app section of [`ARCHITECTURE.md`](../../ARCHITECTURE.md), the app setup in [`CONTRIBUTING.md`](../../.github/CONTRIBUTING.md), the environment mapping in [`README.md`](../../README.md), and [`docs/wiki/CI-CD.md`](../wiki/CI-CD.md).

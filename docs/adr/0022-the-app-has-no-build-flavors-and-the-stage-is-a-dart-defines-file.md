...
That convention is sized for a real difference between stages. When this was written the difference was **one string**; it is two since [27](0027-the-app-sends-telemetry-through-two-ports-with-no-failure-channel.md) added `TELEMETRY_ENVIRONMENT`, which does not reopen this decision because the rule that mattered was that no difference needs Gradle. `dart-defines.json` and `dart-defines.prod.json` each contain a single key, `REVENUECAT_KEY`, and [`main.dart`](../../app/lib/main.dart) reads it with `String.fromEnvironment`. Nothing else about the two builds differs: same `applicationId`, same signing config, same assets, same entitlements, same endpoints, because the app talks to GitHub's public page and to no backend of ours ([5](0005-scrape-githubs-public-contributions-html.md), [11](0011-keep-the-apps-own-scraper-for-now.md)).
...
Locally that is `flutter run --dart-define-from-file=dart-defines.json` for the sandbox and `dart-defines.prod.json` for production. In CI it is the `track` input to [`release-app.yml`](../../.github/workflows/release-app.yml), which selects the GitHub Environment whose `REVENUECAT_KEY` secret is written into `dart-defines.json` at build time.
...
- Where it bites: the **Gotchas** in [`CLAUDE.md`](../../CLAUDE.md), the app section of [`ARCHITECTURE.md`](../../ARCHITECTURE.md), the app setup in [`CONTRIBUTING.md`](../../CONTRIBUTING.md), the environment mapping in [`README.md`](../../README.md), and [`docs/wiki/CI-CD.md`](../wiki/CI-CD.md).

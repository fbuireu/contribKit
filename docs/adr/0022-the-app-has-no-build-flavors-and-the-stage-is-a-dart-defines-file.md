...
- **Forgetting `--dart-define-from-file` is not an error.** `String.fromEnvironment` returns an empty string, RevenueCat is configured with nothing, `offerings.current` is null, and the Tip Jar renders as unavailable. That is a legitimate state ([9](0009-tips-are-unconditional-and-unlock-nothing.md) makes the app fully functional without Tips), so a missing flag looks like a working build with an empty Tip Jar.

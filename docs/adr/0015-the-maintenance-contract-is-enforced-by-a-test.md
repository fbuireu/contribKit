...
- **The triggers in both CI workflows are load-bearing.** Removing `docs/**`, `shared/**` or `*.md` from `ci-web.yml`, or the `Docs Contract` job from `ci-app.yml`, disables the contract for exactly the changes most likely to break it — silently, which is the same failure mode the test exists to prevent.

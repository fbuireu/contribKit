...
- The taxonomies do not match one-for-one, and mostly should not: the app exports files and takes Tips, so it carries `Export`, `Tip` and `Cache` failures the web has no use for. What did have to be reconciled was `Parse` — the app lacked it and reported unparseable markup as `NotFound`, telling a user their account did not exist when GitHub had changed its page.

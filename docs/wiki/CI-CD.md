...
|----------|-------------|------|
| `ci.yml` | every push and PR to `main`, plus manual dispatch, where the dispatch redeploys production and the smoke run behind it (the build inlines the public env, so a rotated analytics token reaches nothing until something redeploys) and cuts no release unless its `release` input is checked, which is how a web release refused by a commit landing on `main` mid-run is cut again without a filler commit | a `changes` job diffs the range and exposes `app`, `web` and `cross_package`; everything else is gated on it. Docs contract, the two per-client workflows, then deploy, the preview comment, the preview e2e, the production smoke run and the release. A final `Check` job aggregates them all |
...
1. **Version:** semantic-release computes the next version from Conventional Commits, updates the changelog, tags `app-vX.Y.Z`. semantic-release itself reports whether it published, through a `successCmd` that writes `published` and `version` to the step's outputs; a `git log` grep used to decide that and silently stopped matching the day the release commit was renamed.

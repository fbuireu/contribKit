...
**Prerequisites:** Node `26.7.0` (the same in the root `engines`, in `web/engines` and in [`web/.nvmrc`](../../web/.nvmrc), which is what CI installs) plus `pnpm@11.21.0`, and, for the app, Flutter `3.47.0` / Dart `3.13.0` exactly as pinned in [`app/pubspec.yaml`](../../app/pubspec.yaml). [lefthook](Git-Hooks) comes with `pnpm install`, which wires the local checks up for you.

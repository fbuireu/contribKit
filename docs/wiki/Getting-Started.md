...
**Prerequisites:** Node at the version [`.nvmrc`](https://github.com/fbuireu/contribKit/blob/main/.nvmrc) declares (the same digit sits in the root `engines` and in `web/engines`, and it is what CI installs) plus the pnpm the root `packageManager` names, and, for the app, the Flutter that `environment.flutter` pins in [`app/pubspec.yaml`](https://github.com/fbuireu/contribKit/blob/main/app/pubspec.yaml), whose Dart is the one `environment.sdk` accepts. [lefthook](Git-Hooks) comes with `pnpm install`, which wires the local checks up for you.

...
| `api/health.ts` | `GET /api/health` — config presence check (`Cache-Control: no-store`) |
| `user/[username].svg.ts` | `GET /user/:username.svg` — returns SVG image |
| `404.astro`, `500.astro` | Error pages — both render the shared `ErrorView` |
...
- Cache headers: `public, max-age=3600, stale-while-revalidate=86400` on all data responses (`api/health` is the exception: `no-store`).

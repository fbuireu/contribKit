...
const run = (path: string, next: () => Promise<Response>, ip = "1.2.3.4"): Promise<Response> =>
...
describe("api rate limiting", () => {
	...
	it("returns 429 when the limit is exceeded", async () => {
		...
		const next = vi.fn(ok);
		const response = await run("/api/contributions", next);
	...
	it("allows requests under the limit, keyed by the client IP", async () => {
		...
		env.API_RATE_LIMITER = { limit };
		const response = await run("/api/health", ok, "9.9.9.9");

...
- `application/http/failure-http.ts` is the single source of truth for mapping `Failure` → HTTP status and message; `isFailure` lives in `domain/failures/failure.ts`. Never redeclare them inline.

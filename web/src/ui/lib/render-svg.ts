import { MONTHS, DOW, type Cell, type RenderCalendarParams } from './calendar-utils';
...
export function renderCalendarString({
  ...
  for (let weekIndex = 0; weekIndex < 53; weekIndex++)
    weeks.push(cells.slice(weekIndex * 7, weekIndex * 7 + 7));
  ...
  weeks.forEach((week, weekIndex) => {
    week.forEach((cell, dayIndex) => {
      ...
      if (shape === 'dot') {
        ...
        parts.push(
          `<circle cx="${x + size / 2}" cy="${y + size / 2}" r="${dotRadius}" fill="${fill}"${data}/>`,
        );
      } else if (shape === 'hex') {
        ...
        for (let i = 0; i < 6; i++) {
          ...
          pts.push(
            `${(centerX + halfSize * Math.cos(a)).toFixed(2)},${(centerY + halfSize * Math.sin(a)).toFixed(2)}`,
          );
        }
        parts.push(`<polygon points="${pts.join(' ')}" fill="${fill}"${data}/>`);
      } else {
        parts.push(
          `<rect x="${x}" y="${y}" width="${size}" height="${size}" rx="${radius}" fill="${fill}"${data}/>`,
        );
      }
    });
  });
...
export function shapePreviewSVG(kind: string): string {
  ...
  if (kind === 'hex') {
    ...
    for (let i = 0; i < 6; i++) {
      const a = (Math.PI / 3) * i + Math.PI / 6;
      pts.push(
        `${(10 + hexSize * Math.cos(a)).toFixed(2)},${(10 + hexSize * Math.sin(a)).toFixed(2)}`,
      );
    }

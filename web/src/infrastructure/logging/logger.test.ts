...
describe("the platform is the transport", () => {
	it("writes one JSON line per call, which is what Cloudflare exports and attributes to the active span", () => {
		logger.info({ message: "contributions fetched", context: { username: "octocat" } });
...
describe("a caller cannot relabel its own line", () => {
	it("keeps the level the method decided, whatever the context says", () => {
		logger.error({ message: "fetch failed", context: { level: LOG_LEVEL.INFO } });
...
describe("logger.logError", () => {
	...
	it("carries the own enumerable fields of an Error subclass beside the standard three", () => {
		const error = Object.assign(new Error("x"), { code: "rate_limited" });

		logger.logError({ message: "test", error });

		expect(lineFrom(LOG_LEVEL.ERROR).error.code).toBe("rate_limited");
	});
...
describe("url redaction", () => {
	it("strips the query string off a url in the log context, so nothing a caller pastes after the ? reaches the sink", () => {
		logger.error({
			message: "fetch failed",
			context: {
				url: "https://contribkit.app/user/octocat.svg?token=redacted-in-fixture",
			},
		});

		const [line] = spies.error.mock.calls[0] as [string];
		expect(JSON.parse(line).url).toBe("https://contribkit.app/user/octocat.svg");
	...
	it("strips it on the logError path too, whichever method emitted the line", () => {
		logger.logError({
			message: "fetch failed",
			error: new Error("x"),
			context: { url: "https://contribkit.app/api/contributions?username=octocat" },
		});

		expect(lineFrom(LOG_LEVEL.ERROR).url).toBe("https://contribkit.app/api/contributions");
	});

...
- **The contact endpoint holds no secret.** It sends through the `CONTACT_EMAIL` `send_email` binding, refuses anything that fills its honeypot field with a silent `202`, and answers `502` when Email Routing refuses the send ([ADR 0029](../docs/adr/0029-contact-messages-leave-through-cloudflares-send-email-binding.md)).

...
- **Nothing may land on `main` while `Semantic Release (web)` is running, and a plain merge is enough to break it.** `@semantic-release/git` commits the version bump and the changelog on the sha the job checked out and pushes `HEAD:main`; a commit that reached `main` in the meantime makes that push a non-fast-forward and the job fails after the deploy and the smoke run have already passed. The `release` group below orders the two release jobs against each other and cannot order a merge. That run leaves no tag behind, and re-running the failed job does not release either: semantic-release checks that the branch it holds is up to date with the remote before publishing and stands down when it is not. What cuts the release is a manual dispatch of `ci.yml` on `main` with its `release` input checked: the run redeploys the head of `main`, `smoke` answers for it, and the release job starts from a checkout that *is* up to date, so the version is computed over every commit since the last tag, the refused one included. Left at its default the input changes nothing, so a credential-rotation dispatch still versions nothing. `release-app.yml` needs no such input, since a dispatch is already the only way it runs. The window to respect is from `deploy-production` going green to `Semantic Release (web)` finishing, a few minutes; queue the next merge behind it rather than beside it, and reach for the dispatch when one lands beside it anyway.
...
file in the tree. It cuts no release unless its `release` input is checked, which is the recovery for a
web release a mid-run merge refused, in the CI section below. There are no Worker runtime secrets here

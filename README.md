...
- **CI:** path-filtered workflows — [`ci-app.yml`](.github/workflows/ci-app.yml) runs on `app/**`; [`ci-web.yml`](.github/workflows/ci-web.yml) runs on `web/**` plus `shared/**`, `docs/**` and `*.md`. The documentation-consistency contract lives in the web test suite but asserts things about both clients, so each workflow runs it

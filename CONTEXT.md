...
**Usage Event**:
A named record that a person reached one point in the product, chosen from a fixed set, together with the option it concerns when there is one: the Palette, Cell Shape, Export Format or Year that was chosen, each drawn from a fixed set of its own. It never carries a Username, free text, or Contribution Data, and therefore no personal data.

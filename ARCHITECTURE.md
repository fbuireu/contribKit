...
- **Releases.** semantic-release per component, own tag series (`web-vX.Y.Z`, `app-vX.Y.Z`), configured in
  [`web/.releaserc.json`](./web/.releaserc.json) and [`app/.releaserc.json`](./app/.releaserc.json). **The release commit is scoped to the package it releases**, `chore(contribkit-web): release …`, because lefthook's `commit-msg` hook runs on it like any other commit and `@commitlint/config-pnpm-scopes` allows only workspace package names. It said `chore(release): web …` until 2026-08-28, which the hook rejected, so `@semantic-release/git` failed after the tag and the GitHub release already existed. Nothing caught it for six days because no commit in between cut a release. **And the rename had a second consumer nobody updated**: `release-app.yml` decided whether to deliver by grepping the last commit for the *old* prefix, so from that day every app release tagged a version, published it on GitHub and skipped Google Play, in a green run. Version 1.4.0 shipped that way. The workflow no longer reads the commit at all: `@semantic-release/exec`'s `successCmd` writes `published` and `version` to `$GITHUB_OUTPUT`, so the answer comes from the tool that knows it rather than from a string kept in step by hand, and the docs contract asserts no workflow spells a release-commit prefix.

...
`_calculateStreak` walks backwards from today through a `DateTime`-keyed map, and **steps a day with
`DateTime(y, m, d - 1)`, never `subtract(Duration(days: 1))`**. Subtracting 24 absolute hours across a daylight-saving
boundary lands on `23:00` or `01:00` instead of midnight, the map lookup misses, and the streak silently stops at the
last clock change. The scraper's grid walk had the identical defect; `infrastructure/CLAUDE.md` carries the long
version.

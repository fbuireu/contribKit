...
- **`viewerProvider` auto-disposes, so every write after an `await` is guarded by `ref.mounted`.** Its watchers are
  `ViewerScreen` and `CustomizerSheet`, and the screen is `main.dart`'s `home:` child and never unmounts, so the
  running app has no path to disposal today. The guards are for the shape of the code rather than a live bug: a
  `Future.microtask` off `build()` and four unawaited async methods, any of which reaches `state =` after a gap.
...
- **Which of those guards is load-bearing depends on what surrounds it.** The ones inside a `try` are defensive:
  a dead-Ref throw lands in the sibling `catch`, whose own write is guarded, so nothing escapes either way. The
  ones that matter are `_loadSettings`'s entry and `finally`, and `fetchContributions`'s `finally`, because
  nothing catches after them and both methods are called without being awaited. `viewer_notifier_test.dart`
  disposes the container at three points to pin exactly those.
...
- **`_loadSettings` swallows the *settings* read** (`catch (_) {}`). A corrupt settings box degrades to defaults
  ...
  **An empty Palette list is a failure, not a quiet nothing.** `_loadPalettes` sets `AssetFailure` when the
